...
namespace social_media.API.Models.Context
{
    public class InitialData
    {
        public static void Initialize(IApplicationBuilder app)
        {
            using (var serviceScope = app.ApplicationServices.GetService<IServiceScopeFactory>().CreateScope())
            {
                ...
                var ailments = InitialData.GetAilments().ToArray();
                context.Ailments.AddRange(ailments);
                context.SaveChanges();

                var medications = InitialData.GetMedications().ToArray();
                context.Medications.AddRange(medications);
                context.SaveChanges();

                var patients = InitialData.GetPatients(context).ToArray();
